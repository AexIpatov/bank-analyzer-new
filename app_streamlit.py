...
def parse_csob_direct(df: pd.DataFrame, account_name: str) -> List[Dict]:
    """Парсер CSOB с заголовками в первой строке"""
    transactions = []
...
def parse_excel(file_content: bytes, filename: str) -> List[Dict]:
    ...
    try:
        # Пробуем прочитать файл CSOB с заголовками в первой строке
        try:
            ...
            if 'account number' in col_names and 'account currency' in col_names:
                transactions = parse_csob_direct_with_headers(df_header, account_name)
                if transactions:
                    return transactions
        except:
            pass
        ...
        for sheet_name, df in sheets.items():
            ...
            if file_type == 'csob':
                transactions = parse_csob_direct(df, account_name)
...
def parse_csob_direct_with_headers(df: pd.DataFrame, account_name: str) -> List[Dict]:
