...
def parse_unicredit(df: pd.DataFrame, account_name: str) -> List[Dict]:
    """Специальный парсер для выписок UniCredit (формат с From Account)"""
    transactions = []
    
    # Ищем строку с заголовками "From Account" по всему файлу
    header_row = -1
    ...
    # Если не нашли, ищем по наличию "Booking Date" и "Amount"
    if header_row == -1:
        for idx in range(min(50, len(df))):
            row_text = ' '.join(str(v).lower() for v in df.iloc[idx].values if pd.notna(v))
    ...
    if header_row >= 0:
        ...
        # Если заголовки не найдены, пробуем найти данные по индексам
        # В этом файле данные начинаются со строки 3 (индекс 3)
        if len(df) > 4:
            # Пропускаем первые 3 строки (Account Title, Account, Currency, Balance)
            df = df.iloc[3:].reset_index(drop=True)
            # Первая строка данных - заголовки
            if len(df) > 0:
    ...
    for col in df.columns:
        ...
        if 'booking date' in col_lower or 'value date' in col_lower or 'date' in col_lower or 'дата' in col_lower:
            if date_col is None:
                date_col = col
            ...
            desc_col = col
        elif 'name' in col_lower or 'контрагент' in col_lower or 'account' in col_lower:
            counterparty_col = col
    
    # Если дата не найдена, ищем по содержимому
    if date_col is None:
        for col in df.columns:
    ...
    # Если сумма не найдена, ищем по содержимому
    ...
    # Если сумма не найдена, ищем по позиции (вторая колонка)
