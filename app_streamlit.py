...
def detect_file_type(df: pd.DataFrame, filename: str) -> str:
    ...
    if 'revolut' in filename_lower:
        return 'revolut'
    if 'paysera' in filename_lower:
        return 'paysera'
    if 'unicredit' in filename_lower or 'uc' in filename_lower:
        return 'unicredit'
    if 'csob' in filename_lower:
        return 'csob'
    if 'industra' in filename_lower:
        return 'industra'
    ...
    if 'mashreq' in filename_lower:
        return 'mashreq'
    if 'wio' in filename_lower:
        return 'wio'
    if 'mkb' in filename_lower or 'budapest' in filename_lower:
        return 'mkb'
    if 'fio' in filename_lower:
        return 'fio'
    if 'wise' in filename_lower:
        return 'wise'
    if 'n26' in filename_lower:
        return 'n26'
    if 'tinkoff' in filename_lower:
        return 'tinkoff'
    if 'alpha' in filename_lower or 'alfa' in filename_lower:
        return 'alfa'
    if 'rak bank' in filename_lower:
        return 'rak'
    
    # Проверяем по содержимому первых строк
    for idx in range(min(10, len(df))):
        ...
        if 'debit' in row_text and 'credit' in row_text:
            return 'industra'
        if 'hesab üzrə çıxarış' in row_text or 'mədaxil' in row_text:
            return 'pasha'
        if 'account transactions statement' in row_text:
            return 'mashreq'
        if 'account name' in row_text and 'account iban' in row_text:
            return 'wio'
        if 'számlaszám' in row_text or 'értéknap' in row_text:
            return 'mkb'
...
def parse_unicredit(df: pd.DataFrame, account_name: str) -> List[Dict]:
    ...
    if header_row == -1:
        for idx in range(min(30, len(df))):
            row_text = ' '.join(str(v).lower() for v in df.iloc[idx].values if pd.notna(v))
            if 'booking date' in row_text and 'amount' in row_text:
                header_row = idx
                break
    ...
    if header_row >= 0:
        ...
        # Пробуем пропустить первые строки
        if len(df) > 4:
            df = df.iloc[3:].reset_index(drop=True)
            if len(df) > 0:
                headers = [str(h).strip() if pd.notna(h) else f'col_{i}' for i, h in enumerate(df.iloc[0].values)]
                data_rows = []
    ...
    for col in df.columns:
        col_lower = str(col).lower()
        if 'booking date' in col_lower or 'value date' in col_lower or 'date' in col_lower:
            if date_col is None:
                date_col = col
        elif 'amount' in col_lower:
            amount_col = col
...
def parse_industra(df: pd.DataFrame, account_name: str) -> List[Dict]:
    ...
    for idx in range(min(30, len(df))):
        ...
        if any(kw in row_text for kw in ['дебет', 'debit', 'кредит', 'credit']):
            if any(kw in row_text for kw in ['дата', 'date']):
                header_row = idx
                break
    ...
    for col in df.columns:
        col_lower = str(col).lower()
        if any(kw in col_lower for kw in ['дата', 'date', 'datum']):
            date_col = col
        elif any(kw in col_lower for kw in ['дебет', 'debit']):
            debit_col = col
        elif any(kw in col_lower for kw in ['кредит', 'credit']):
            credit_col = col
        elif any(kw in col_lower for kw in ['информация', 'описание', 'description']):
            desc_col = col
...
def parse_fio(df: pd.DataFrame, account_name: str) -> List[Dict]:
    ...
    for col in df.columns:
        ...
        if 'date' in col_lower:
            ...
            amount_col = col
        elif 'message for beneficiary' in col_lower or 'note' in col_lower:
            desc_col = col
...
def parse_generic(df: pd.DataFrame, account_name: str) -> List[Dict]:
    ...
    for idx in range(min(30, len(df))):
        row_text = ' '.join(str(v).lower() for v in df.iloc[idx].values if pd.notna(v))
        if any(kw in row_text for kw in ['date', 'дата', 'amount', 'сумма']):
            header_row = idx
            break
    ...
    for col in df.columns:
        ...
        if any(kw in col_lower for kw in ['date', 'дата', 'datum']):
            if date_col is None:
                date_col = col
        elif any(kw in col_lower for kw in ['amount', 'сумма', 'volume']):
            if amount_col is None:
                amount_col = col
        elif any(kw in col_lower for kw in ['description', 'описание', 'note']):
            if desc_col is None:
                desc_col = col
...
def parse_excel(file_content: bytes, filename: str) -> List[Dict]:
    ...
    try:
        ...
        for sheet_name, df in sheets.items():
            ...
            # Применяем соответствующий парсер
            ...
            elif file_type in ['industra', 'csob', 'bluor']:
